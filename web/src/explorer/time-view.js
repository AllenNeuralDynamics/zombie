...
/**
 * Build a Mosaic Selection clause for a single project name.
 *
 * Returns an object with `source`, `value`, and `predicate` suitable for
 * passing to `Selection.update()`. When no project is given the predicate
 * is `null` so the TimeView shows all rows.
 *
 * @param {string|null|undefined} projectName
 * @returns {{ source: 'project', value: string|null, predicate: object|null }}
 */
export function buildProjectClause(projectName) {
  const value = projectName || null;
  const predicate = value ? eq(column(TIME_COL_PROJECT), literal(value)) : null;
  return { source: 'project', value, predicate };
}
