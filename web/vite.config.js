...
import { selectRoutes } from './build/routes.js';
 ...
 */
function sharedHeaderPlugin(routes) {
  return {
    name: 'shared-header',
    transformIndexHtml(html, ctx) {
      ...
      if (out.includes('<!--APP_HEADER-->')) {
        ...
        if (page) out = out.replace('<!--APP_HEADER-->', renderHeader(page, routes));
      }
      return out;
    },
  };
...
/**
 * Which release channel this invocation builds (see `build/routes.js`).
 *
 * The dev server always serves everything so experimental pages can be worked
 * on. `vite build` ships the stable set unless ZOMBIE_EXPERIMENTAL=1 asks for
 * the dev-channel bundle — the safe default, so forgetting the flag under-ships
 * rather than leaking an unfinished page into production.
 */
function includeExperimental(command) {
  return command === 'serve' || process.env.ZOMBIE_EXPERIMENTAL === '1';
}

export default defineConfig(({ command }) => {
  const routes = selectRoutes({ includeExperimental: includeExperimental(command) });
  ...
  return {
    ...
    plugins: [sharedHeaderPlugin(routes), react({ include: /\.(jsx|tsx)$/ })],
