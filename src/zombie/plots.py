...
@pn.cache()
def raster_aggregated(sx, sy,
              zoom_window=None, t_bins=1000, d_bins=394/2,
    ...
    df = pd.DataFrame(data={'time': x.ravel(), 'depth': y.ravel(), 'count': raster.ravel()})

    raster_plot = alt.Chart(df).mark_rect().encode(
        x=alt.X('time:O', axis=alt.Axis(values=[xscale[1], xscale[-1]], format='.0f', title='Time (index)')),
        y=alt.Y('depth:O', axis=alt.Axis(values=[yscale[1], yscale[-1]], format='.0f', tickCount=10, title='Depth on probe (um)')),
        color=alt.Color('count:Q', scale=alt.Scale(range=['white', 'black']))
    ).properties(
        width=width,
        height=height
    )
