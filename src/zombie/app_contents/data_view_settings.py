...
class DataViewSettings(PyComponent):
    ...
    def __init__(self, **kwargs):
        ...
        self.filter_column_selector = pn.widgets.Select.from_param(
            self.param.filter_column,
            name="Filter Column (optional)"
        )
        self.filter_values_selector = pn.widgets.MultiChoice.from_param(
            self.param.filter_values,
            name="Keep Values"
        )
        
        # Column mapping section
        mapping_section = pn.pane.Markdown("#### Column Mapping")
        self.x_selector = pn.widgets.Select.from_param(
            self.param.x_column,
            name="X Axis"
        )
        self.y_selector = pn.widgets.Select.from_param(
            self.param.y_column,
            name="Y Axis"
        )
        self.by_selector = pn.widgets.Select.from_param(
            self.param.by_column,
            name="Color By (optional)"
        )
        
        # Appearance section
        appearance_section = pn.pane.Markdown("#### Plot Appearance")
        self.xlabel_input = pn.widgets.TextInput.from_param(
            self.param.xlabel,
            name="X Label"
        )
        self.ylabel_input = pn.widgets.TextInput.from_param(
            self.param.ylabel,
            name="Y Label"
        )
        self.title_input = pn.widgets.TextInput.from_param(
            self.param.title,
            name="Title"
        )
        self.width_input = pn.widgets.IntInput.from_param(
            self.param.width,
            name="Width"
        )
        self.height_input = pn.widgets.IntInput.from_param(
            self.param.height,
            name="Height"
        )
