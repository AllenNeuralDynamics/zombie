...
class TimelineView(PyComponent):
    ...
    def _update_plot(self):
        ...
        # Convert ALL timestamps to timezone-naive UTC for plotting
        # Original timezone info is preserved in the 'data' field for details display
        ...
        # Calculate x-axis bounds with padding to prevent scrolling past events
        min_date = df["start"].min()
        max_date = df["end"].max()
        padding = timedelta(days=3)  # Add 3 days padding on each side
        x_min = min_date - padding
        x_max = max_date + padding

        # Create timeline plot using rectangles
        rectangles = (
            hv.Rectangles(rect_df, kdims=["x0", "y0", "x1", "y1"], vdims=["event", "type", "details", "age_days"])
            .opts(
                color="type",
                cmap="Category20",
                tools=["hover", "tap"],
        ...
        labels = hv.Labels(label_df, kdims=["x", "label_y"], vdims=["event"]).opts(
            angle=45, text_font_size="10pt", text_align="left"
        )
    ...
    def _on_tap(self, *events):
        ...
        # Convert tap_stream.x to timezone-naive timestamp to match events_df
        ...
        # Convert to timezone-naive UTC to match events_df
        if tap_time.tz is not None:
            tap_time = tap_time.tz_convert("UTC").tz_localize(None)
...
"""
            ...
            tabs = pn.Tabs(
                ("Overview", overview_pane), ("Fiber Locations", fiber_viz_pane), sizing_mode="stretch_width"
            )
